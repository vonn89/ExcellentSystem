...
public class LaporanTerimaServisController {
    ...
    @FXML private ComboBox<String> statusLunasCombo;
    @FXML private ComboBox<String> statusBatalCombo;
    ...
    public void initialize(){
        ...
        ObservableList<String> statusLunas = FXCollections.observableArrayList();
        statusLunas.add("Semua");
        statusLunas.add("Sudah Diambil");
        statusLunas.add("Belum Diambil");
        statusLunasCombo.setItems(statusLunas);
        statusLunasCombo.getSelectionModel().select("Semua");
        
        ObservableList<String> statusBatal = FXCollections.observableArrayList();
        statusBatal.add("Semua");
        statusBatal.add("Servis");
        statusBatal.add("Batal Servis");
        statusBatalCombo.setItems(statusBatal);
        statusBatalCombo.getSelectionModel().select("Semua");
    ...
    private void getBarang(){
        try(Connection conCabang = KoneksiCabang.getConnection(cabang)){
            String statusBatal = "%";
            if(statusBatalCombo.getSelectionModel().getSelectedItem().equals("Semua"))
                statusBatal = "%";
            else if(statusBatalCombo.getSelectionModel().getSelectedItem().equals("Servis"))
                statusBatal = "false";
            else if(statusBatalCombo.getSelectionModel().getSelectedItem().equals("Batal Servis"))
                statusBatal = "true";
            
            String statusLunas = "%";
            if(statusLunasCombo.getSelectionModel().getSelectedItem().equals("Semua"))
                statusLunas = "%";
            else if(statusLunasCombo.getSelectionModel().getSelectedItem().equals("Sudah Diambil"))
                statusLunas = "true";
            else if(statusLunasCombo.getSelectionModel().getSelectedItem().equals("Belum Diambil"))
                statusLunas = "false";
            
            List<Servis> listHutang = ServisDAO.getAllByDateAndStatus(
                conCabang, tglAwalPicker.getValue().toString(), tglAkhirPicker.getValue().toString(), statusLunas, statusBatal);
            List<Servis> filterData = filterData(listHutang);
//            filterData.sort(Comparator.comparing(Servis::getTotalHutang));
            JasperDesign jasperDesign = JRXmlLoader.load(getClass().getResourceAsStream("LaporanTerimaServis.jrxml"));
            ...
            hash.put("statusBatal", statusBatalCombo.getSelectionModel().getSelectedItem());
            hash.put("statusLunas", statusLunasCombo.getSelectionModel().getSelectedItem());
