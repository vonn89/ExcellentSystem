...
public class LaporanStokHutangPeriodeController {
    ...
    public void initialize(){
        ...
        tglMulaiPicker.setDayCellFactory((final DatePicker datePicker) -> 
                Function.getDateCellDisableAfter(tglAkhirPicker.getValue()));
        tglAkhirPicker.setConverter(Function.getTglConverter());
        tglAkhirPicker.setValue(LocalDate.parse(sistem.getTglSystem()));
        tglAkhirPicker.setDayCellFactory((final DatePicker datePicker) -> 
                Function.getDateCellDisableBefore(tglMulaiPicker.getValue()));
    ...
    private void getBarang(){
        try(Connection conCabang = KoneksiCabang.getConnection(cabang)){
            List<StokHutang> listStokAwal = StokHutangDAO.getAllByTanggal(conCabang, tglMulaiPicker.getValue().toString());
            List<StokHutang> listStokAkhir = StokHutangDAO.getAllByTanggal(conCabang, tglAkhirPicker.getValue().toString());
            List<StokHutang> listHutang = StokHutangDAO.getSumByPeriode(conCabang, tglMulaiPicker.getValue().toString(), tglAkhirPicker.getValue().toString());
            for(StokHutang s : listHutang){
                for(StokHutang awal : listStokAwal){
                    if(s.getTglHutang().equals(awal.getTglHutang())){
                        s.setStokAwal(awal.getStokAwal());
                        s.setBeratAwal(awal.getBeratAwal());
                        s.setJumlahAwal(awal.getJumlahAwal());
                    }
                }
                for(StokHutang akhir : listStokAkhir){
                    if(s.getTglHutang().equals(akhir.getTglHutang())){
                        s.setStokAkhir(akhir.getStokAkhir());
                        s.setBeratAkhir(akhir.getBeratAkhir());
                        s.setJumlahAkhir(akhir.getJumlahAkhir());
                    }
                }
            }
            List<StokHutang> filterData = filterData(listHutang);
            filterData.sort(Comparator.comparing(StokHutang::getTglHutang, Comparator.reverseOrder()));
            JasperDesign jasperDesign = JRXmlLoader.load(getClass().getResourceAsStream("LaporanStokHutang.jrxml"));
            ...
            hash.put("tanggal", tglNormal.format(tglBarang.parse(tglMulaiPicker.getValue().toString()))+"-"+tglNormal.format(tglBarang.parse(tglAkhirPicker.getValue().toString())));
    ...
    private List<StokHutang> filterData(List<StokHutang> listData) throws Exception{
        List<StokHutang> filterData = new ArrayList<>();
        for (StokHutang p : listData) {
            if (searchField.getText() == null || searchField.getText().equals(""))
                filterData.add(p);
            else{
                if(checkColumn(tglNormal.format(tglBarang.parse(p.getTglHutang()))))
                    filterData.add(p);
            }
        }
        return filterData;
