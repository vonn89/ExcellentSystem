...
public class LaporanPembelianCabangController {
    ...
    private void getBarang(){
        try(Connection conPusat = KoneksiPusat.getConnection()){
            String cabangAsal = cabangCombo.getSelectionModel().getSelectedItem();
            if(cabangCombo.getSelectionModel().getSelectedItem().equals("Semua"))
                cabangAsal = "%";
            List<PenjualanAntarCabangDetail> listPembelianDetail = PenjualanAntarCabangDetailDAO.getAllByTglTerimaAndCabangAsalAndCabangTujuan(
                conPusat, tglAwalPicker.getValue().toString(), tglAkhirPicker.getValue().toString(), cabangAsal, cabang.getKodeCabang());
            for(PenjualanAntarCabangDetail d : listPembelianDetail){
                d.setPenjualanAntarCabangHead(PenjualanAntarCabangHeadDAO.get(conPusat, d.getNoPenjualan()));
            }
            List<PenjualanAntarCabangDetail> filterData = filterData(listPembelianDetail);
//            listBarang.sort(Comparator.comparing(Barang::getKodeKategori));
            Collections.sort(filterData, (o1, o2) -> {
                int sComp = ((PenjualanAntarCabangDetail) o1).getNoPenjualan().compareTo(((PenjualanAntarCabangDetail) o2).getNoPenjualan());
                if (sComp != 0) 
                    return sComp;
                return ((PenjualanAntarCabangDetail) o1).getKodeJenis().compareTo(((PenjualanAntarCabangDetail) o2).getKodeJenis());
            });
            JasperDesign jasperDesign = JRXmlLoader.load(getClass().getResourceAsStream("LaporanPembelianCabang.jrxml"));
            ...
            hash.put("cabangAsal", cabangCombo.getSelectionModel().getSelectedItem());
        ...
        for (PenjualanAntarCabangDetail p : listData) {
            if (searchField.getText() == null || searchField.getText().equals(""))
                ...
                if(checkColumn(p.getNoPenjualan())||
                    ...
                    checkColumn(p.getPenjualanAntarCabangHead().getKodeCabang())||
                    ...
                    checkColumn(rp.format(p.getPenjualanAntarCabangHead().getTotalBerat()))||
