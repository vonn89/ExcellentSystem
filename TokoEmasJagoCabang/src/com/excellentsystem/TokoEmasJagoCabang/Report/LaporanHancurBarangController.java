...
import com.excellentsystem.TokoEmasJagoCabang.DAO.Pusat.HancurDetailDAO;
import com.excellentsystem.TokoEmasJagoCabang.DAO.Pusat.HancurHeadDAO;
...
import com.excellentsystem.TokoEmasJagoCabang.Model.Pusat.HancurHead;
...
public class LaporanHancurBarangController {
    ...
    private void getBarang(){
        try(Connection conPusat = KoneksiPusat.getConnection()){
            List<HancurHead> listHancurHead = HancurHeadDAO.getAllByDateAndCabangAndStatus(
                    conPusat, tglAwalPicker.getValue().toString(), tglAkhirPicker.getValue().toString(), cabang.getKodeCabang(), "true");
            List<HancurDetail> listHancurDetail = HancurDetailDAO.getAllByDateAndCabangAndStatus(
                    conPusat, tglAwalPicker.getValue().toString(), tglAkhirPicker.getValue().toString(), cabang.getKodeCabang(), "true");
            for(HancurDetail d : listHancurDetail){
                for(HancurHead a : listHancurHead){
                    if(d.getNoHancur().equals(a.getNoHancur()))
                        d.setHancurHead(a);
                }
            }
            List<HancurDetail> filterData = filterData(listHancurDetail);
//            listBarang.sort(Comparator.comparing(Barang::getKodeKategori));
            Collections.sort(filterData, (o1, o2) -> {
                int sComp = ((HancurDetail) o1).getNoHancur().compareTo(((HancurDetail) o2).getNoHancur());
                if (sComp != 0) 
                    return sComp;
                return ((HancurDetail) o1).getKodeJenis().compareTo(((HancurDetail) o2).getKodeJenis());
            });
            JasperDesign jasperDesign = JRXmlLoader.load(getClass().getResourceAsStream("LaporanHancurBarang.jrxml"));
    ...
    private List<HancurDetail> filterData(List<HancurDetail> listData) throws Exception{
        List<HancurDetail> filterData = new ArrayList<>();
        for (HancurDetail p : listData) {
            if (searchField.getText() == null || searchField.getText().equals(""))
                filterData.add(p);
            else{
                if(checkColumn(p.getNoHancur())||
                    ...
                    checkColumn(gr.format(p.getBeratPersenRosok())))
                    filterData.add(p);
            }
        }
        return filterData;
