...
public class LaporanLogMemberController {
    ...
    public void setMember(String kodeMember){
    ...
    private void getBarang(){
        if(kodeMember!=null){
            try(Connection conPusat = KoneksiPusat.getConnection()){
                Member m = MemberDAO.get(conPusat, kodeMember);

                List<LogMember> listLogMember = LogMemberDAO.getAllByDateAndCabangAndMemberAndKategoriAndSalesAndStatus(
                        conPusat, tglAwalPicker.getValue().toString(), tglAkhirPicker.getValue().toString(), "%", kodeMember, "%", "%", "true");
                for(LogMember l : listLogMember){
                    l.setMember(m);
                }

                Map hash = new HashMap();
                hash.put("poinAwal", LogMemberDAO.getPoinBefore(conPusat, kodeMember, tglAwalPicker.getValue().toString()));

                JasperReport jasperReport = JasperCompileManager.compileReport(JRXmlLoader.load(getClass().getResourceAsStream("LogMember.jrxml")));
                JRBeanCollectionDataSource beanColDataSource = new JRBeanCollectionDataSource(listLogMember);
