...
public class LaporanPenjualanController {
    ...
    @FXML private ComboBox<String> statusKurangCombo;
    ...
    public void initialize(){
        ...
        statusKurang.add("Lunas");
        statusKurang.add("Belum Lunas");
        statusKurangCombo.setItems(statusKurang);
        statusKurangCombo.getSelectionModel().select("Semua");
        
        ObservableList<String> statusBatal = FXCollections.observableArrayList();
        statusBatal.add("Semua");
        statusBatal.add("Penjualan");
        statusBatal.add("Batal Penjualan");
        statusBatalCombo.setItems(statusBatal);
        statusBatalCombo.getSelectionModel().select("Penjualan");
    ...
    private void getBarang(){
        try(Connection conCabang = KoneksiCabang.getConnection(cabang)){
            ...
            if(statusBatalCombo.getSelectionModel().getSelectedItem().equals("Semua"))
                status = "%";
            else if(statusBatalCombo.getSelectionModel().getSelectedItem().equals("Penjualan"))
                status = "true";
            else if(statusBatalCombo.getSelectionModel().getSelectedItem().equals("Batal Penjualan"))
                status = "false";
            List<PenjualanDetail> listPenjualanDetail = PenjualanDetailDAO.getAllByDateAndStatus(
                conCabang, tglAwalPicker.getValue().toString(), tglAkhirPicker.getValue().toString(), status);
            ...
            for(PenjualanDetail d : listPenjualanDetail){
                ...
                if(statusKurangCombo.getSelectionModel().getSelectedItem().equals("Semua")){
                    listPenjualan.add(d);
                }else if(statusKurangCombo.getSelectionModel().getSelectedItem().equals("Lunas")){
                    if(d.getPenjualanHead().getSisaPembayaran()==0)
                        listPenjualan.add(d);
                }else if(statusKurangCombo.getSelectionModel().getSelectedItem().equals("Belum Lunas")){
                    if(d.getPenjualanHead().getSisaPembayaran()!=0)
                        listPenjualan.add(d);
                }
            }
            List<PenjualanDetail> filterData = filterData(listPenjualan);
//            listBarang.sort(Comparator.comparing(Barang::getKodeKategori));
            Collections.sort(filterData, (o1, o2) -> {
                int sComp = ((PenjualanDetail) o1).getNoPenjualan().compareTo(((PenjualanDetail) o2).getNoPenjualan());
                if (sComp != 0) 
                    return sComp;
            ...
            hash.put("statusLunas", statusKurangCombo.getSelectionModel().getSelectedItem());
    ...
    private List<PenjualanDetail> filterData(List<PenjualanDetail> listData) throws Exception{
        List<PenjualanDetail> filterData = new ArrayList<>();
        for (PenjualanDetail p : listData) {
            if (searchField.getText() == null || searchField.getText().equals(""))
                ...
                if(checkColumn(p.getNoPenjualan())||
                    ...
                    checkColumn(gr.format(p.getHargaJual()))||
                    checkColumn(gr.format(p.getOngkos())))
                    filterData.add(p);
            }
        }
        return filterData;
