...
public class LaporanTransaksiSalesController {
    ...
    private void getBarang(){
        try(Connection conCabang = KoneksiCabang.getConnection(cabang)){
            ...
            for(Keuangan s : listKeuangan){
                boolean status = false;
                for(Keuangan temp : allKeuangan){
                    if(s.getKodeSales().equalsIgnoreCase(temp.getKodeSales()) && s.getKategori().equalsIgnoreCase(temp.getKategori())){
                        status = true;
                        temp.setJumlahRp(temp.getJumlahRp()+s.getJumlahRp());
                    }
                }
                if(status==false)
                    allKeuangan.add(s);
            }
            Collections.sort(allKeuangan, (o1, o2) -> {
                int sComp = ((Keuangan) o1).getKodeSales().toUpperCase().compareTo(((Keuangan) o2).getKodeSales().toUpperCase());
                if (sComp != 0) 
                    return sComp;
            ...
            JasperReport jasperReport = JasperCompileManager.compileReport(JRXmlLoader.load(getClass().getResourceAsStream("LaporanTransaksiSales.jrxml")));
            JRBeanCollectionDataSource beanColDataSource = new JRBeanCollectionDataSource(allKeuangan);
