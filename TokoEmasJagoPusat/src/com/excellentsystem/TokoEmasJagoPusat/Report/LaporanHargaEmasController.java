...
public class LaporanHargaEmasController {
    ...
    private void getBarang(){
        try(Connection con = KoneksiPusat.getConnection()){
            List<LogHargaEmas> listHargaEmas = LogHargaEmasDAO.getAllByTanggal(con, tglAwalPicker.getValue().toString(), tglAkhirPicker.getValue().toString());
            listHargaEmas.sort(Comparator.comparing(LogHargaEmas::getTanggal));
            JasperDesign jasperDesign = JRXmlLoader.load(getClass().getResourceAsStream("LaporanHargaEmas.jrxml"));
            JRBeanCollectionDataSource beanColDataSource = new JRBeanCollectionDataSource(listHargaEmas);
