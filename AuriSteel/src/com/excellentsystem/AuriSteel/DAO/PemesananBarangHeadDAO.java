 ...
 * @author Xtreme
 */
public class PemesananBarangHeadDAO {
    public static List<PemesananBarangHead> getAllByDateAndStatus(Connection con, 
        ...
        List<PemesananBarangHead> allPemesanan = new ArrayList<>();
        ResultSet rs = ps.executeQuery();
        while(rs.next()){
            PemesananBarangHead p = new PemesananBarangHead();
    ...
    public static PemesananBarangHead get(Connection con, String noPemesanan)throws Exception{
        PreparedStatement ps = con.prepareStatement("select * from tt_pemesanan_head where no_pemesanan = ? ");
        ps.setString(1, noPemesanan);
        PemesananBarangHead p = null;
        ResultSet rs = ps.executeQuery();
        while(rs.next()){
            p = new PemesananBarangHead();
            p.setNoPemesanan(rs.getString(1));
            p.setTglPemesanan(rs.getDate(2).toString()+" "+rs.getTime(2).toString());
            p.setKodeCustomer(rs.getString(3));
            p.setKodeCustomerInvoice(rs.getString(4));
    ...
    public static void insert(Connection con, PemesananBarangHead p)throws Exception{
    ...
    public static void update(Connection con, PemesananBarangHead p)throws Exception{
