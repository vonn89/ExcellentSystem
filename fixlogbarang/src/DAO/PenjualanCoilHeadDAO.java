...
import Model.PenjualanCoilHead;
 ...
 * @author Xtreme
 */
public class PenjualanCoilHeadDAO {
    public static List<PenjualanCoilHead> getAllByNoPemesananAndStatus(
        ...
        List<PenjualanCoilHead> allPenjualan = new ArrayList<>();
        ResultSet rs = ps.executeQuery();
        while(rs.next()){
            PenjualanCoilHead p = new PenjualanCoilHead();
    ...
    public static List<PenjualanCoilHead> getAllByDateAndStatus(
    ...
    public static PenjualanCoilHead get(Connection con, String noPenjualan)throws Exception{
        ...
        ResultSet rs = ps.executeQuery();
        PenjualanCoilHead p = null;
        while(rs.next()){
            p = new PenjualanCoilHead();
            p.setNoPenjualan(rs.getString(1));
            p.setTglPenjualan(rs.getDate(2).toString()+" "+rs.getTime(2).toString());
            p.setNoPemesanan(rs.getString(3));
            p.setKodeCustomer(rs.getString(4));
    ...
    public static String getId(Connection con)throws Exception{
        ...
        ResultSet rs = ps.executeQuery();
        if(rs.next())
            return "PE-"+yymm.format(serverDate)+new DecimalFormat("000").format(rs.getInt(1)+1);
        else
            return "PE-"+yymm.format(serverDate)+new DecimalFormat("000").format(1);
    }
    public static void insert(Connection con, PenjualanCoilHead p)throws Exception{
    ...
    public static void update(Connection con, PenjualanCoilHead p)throws Exception{
