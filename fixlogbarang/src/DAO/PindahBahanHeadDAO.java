 ...
 * @author ASUS
 */
public class PindahBahanHeadDAO {
    
    public static List<PindahBahanHead> getAllByDateAndStatus(Connection con, String tglMulai,String tglAkhir, String status)throws Exception{
        String sql = "select * from tt_pindah_bahan_head where left(tgl_pindah,10) between ? and ? ";
        if(!status.equals("%"))
            sql = sql + " and status = '"+status+"' ";
        PreparedStatement ps = con.prepareStatement(sql);
        ps.setString(1, tglMulai);
        ps.setString(2, tglAkhir);
        List<PindahBahanHead> allPenjualan = new ArrayList<>();
    ...
    public static PindahBahanHead get(Connection con, String noPenjualan)throws Exception{
        PreparedStatement ps = con.prepareStatement("select * from tt_pindah_bahan_head where no_pindah = ?");
        ps.setString(1, noPenjualan);
        ResultSet rs = ps.executeQuery();
        PindahBahanHead p = null;
        while(rs.next()){
            p = new PindahBahanHead();
            p.setNoPindah(rs.getString(1));
            p.setTglPindah(rs.getDate(2).toString()+" "+rs.getTime(2).toString());
            p.setGudangAsal(rs.getString(3));
            p.setGudangTujuan(rs.getString(4));
    ...
    public static String getId(Connection con)throws Exception{
        ...
        PreparedStatement ps = con.prepareStatement("select max(right(no_pindah,3)) from tt_pindah_bahan_head "
                + " where mid(no_pindah,4,4) = ?");
        ps.setString(1, yymm.format(serverDate));
        ResultSet rs = ps.executeQuery();
        if(rs.next())
            return "PH-"+yymm.format(serverDate)+new DecimalFormat("000").format(rs.getInt(1)+1);
        else
            return "PH-"+yymm.format(serverDate)+new DecimalFormat("000").format(1);
    }
    public static void insert(Connection con, PindahBahanHead p)throws Exception{
        PreparedStatement ps = con.prepareStatement("insert into tt_pindah_bahan_head values(?,?,?,?,?,?,?,?,?,?)");
    ...
    public static void update(Connection con, PindahBahanHead p)throws Exception{
        PreparedStatement ps = con.prepareStatement("update tt_pindah_bahan_head set "
