...
public class KategoriPiutangDAO {
    public static void insert(Connection con, KategoriPiutang k)throws Exception{
        try (PreparedStatement ps = con.prepareStatement("insert into tm_kategori_piutang values(?)")) {
            ps.setString(1, k.getKodeKategori());
            ps.executeUpdate();
        }
    }
    public static void delete(Connection con, KategoriPiutang k)throws Exception{
        try (PreparedStatement ps = con.prepareStatement("delete from tm_kategori_piutang where kode_kategori=?")) {
            ps.setString(1, k.getKodeKategori());
            ps.executeUpdate();
        }
    }
    public static List<KategoriPiutang> getAll(Connection con)throws Exception{
        ...
        try (PreparedStatement ps = con.prepareStatement("select * from tm_kategori_piutang")) {
