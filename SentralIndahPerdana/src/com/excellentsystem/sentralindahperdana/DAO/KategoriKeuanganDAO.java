...
public class KategoriKeuanganDAO {
    ...
    public static void insert(Connection con, KategoriKeuangan k)throws Exception{
        try (PreparedStatement ps = con.prepareStatement("insert into tm_kategori_keuangan values(?)")) {
            ps.setString(1, k.getKodeKeuangan());
            ps.executeUpdate();
        }
    }
    public static void delete(Connection con, KategoriKeuangan k)throws Exception{
        try (PreparedStatement ps = con.prepareStatement("delete from tm_kategori_keuangan where kode_keuangan=?")) {
            ps.setString(1, k.getKodeKeuangan());
            ps.executeUpdate();
        }
    }
    public static List<KategoriKeuangan> getAll(Connection con)throws Exception{
        ...
        try (PreparedStatement ps = con.prepareStatement("select * from tm_kategori_keuangan")) {
