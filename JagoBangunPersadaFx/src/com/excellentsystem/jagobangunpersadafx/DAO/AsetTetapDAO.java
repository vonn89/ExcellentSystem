...
public class AsetTetapDAO {
    ...
    public static List<AsetTetap> getAllByStatus(Connection con, String status)throws Exception{
        PreparedStatement ps = con.prepareStatement("select * from tm_aset_tetap where status like ? ");
    ...
    public static String getId(Connection con)throws Exception{
        PreparedStatement ps = con.prepareStatement("select max(right(no_aset_tetap,3)) from tm_aset_tetap where mid(no_aset_tetap,4,4) = ?");
        ps.setString(1, new SimpleDateFormat("yyMM").format(new Date()));
        ResultSet rs = ps.executeQuery();
        if(rs.next())
            return "AT-"+new SimpleDateFormat("yyMM").format(new Date())+new DecimalFormat("000").format(rs.getInt(1)+1);
        else
            return "AT-"+new SimpleDateFormat("yyMM").format(new Date())+new DecimalFormat("000").format(1);
    }
