 ...
 * @author Xtreme
 */
public class LaporanPiutangController  {
        ...
        Task<List<Keuangan>> task = new Task<List<Keuangan>>() {
            ...
            public List<Keuangan> call()throws Exception {
                try (Connection con = Koneksi.getConnection()) {
                    saldoAwal = KeuanganDAO.getSaldoAwal(con, tglMulaiPicker.getValue().toString(),"Piutang");
                    saldoAkhir = KeuanganDAO.getSaldoAkhir(con, tglAkhirPicker.getValue().toString(),"Piutang");
                    return KeuanganDAO.getAllByTipeKeuanganAndTanggal(con, 
                        "Piutang", tglMulaiPicker.getValue().toString(), tglAkhirPicker.getValue().toString());
                }
            }
        };
        task.setOnRunning((e) -> {
    ...
    private void print(){
        try{
            ...
            report.printLaporanPiutang(allKeuangan, tglMulaiPicker.getValue().toString(), tglAkhirPicker.getValue().toString(),
    ...
    private void exportExcel(){
        try{
            ...
            if (file != null) {
                ...
                Sheet sheet = workbook.createSheet("Laporan Piutang");
                int rc = 0;
                int c = 5;
