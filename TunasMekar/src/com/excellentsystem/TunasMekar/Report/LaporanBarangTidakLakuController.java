...
public class LaporanBarangTidakLakuController {
    ...
    private void getBarang(){
        try(Connection con = Koneksi.getConnection()){
            ...
            for(Barang b : listBarang){
                double qty = 0;
                for(PenjualanDetail s : listPenjualanDetail){
                    if(s.getKodeBarang().equals(b.getKodeBarang()))
                        qty = qty + s.getQty();
                }
                if(qty==0){
                    b.setStokAkhir(StokBarangDAO.get(con, sistem.getTglSystem(), b.getKodeBarang()));
                    data.add(b);
                }
            }
            List<Barang> filterBarang = filterData(data);
            Collections.sort(filterBarang, (o1, o2) -> {
                return ((Barang) o1).getKodeKategori().compareTo(
                        ((Barang) o2).getKodeKategori());
            });
            JasperDesign jasperDesign = JRXmlLoader.load(getClass().getResourceAsStream("LaporanBarangTidakLaku.jrxml"));
            ...
            hash.put("tanggal", tglNormal.format(tglBarang.parse(tglAwalPicker.getValue().toString()))+" - "+
                    tglNormal.format(tglBarang.parse(tglAwalPicker.getValue().toString())));
        ...
        for (Barang b : listBarang) {
            if (searchField.getText() == null || searchField.getText().equals(""))
                ...
                if(checkColumn(b.getKodeBarang())||
                    checkColumn(b.getKodeKategori())||
                    checkColumn(b.getSupplier())||
