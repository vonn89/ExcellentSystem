...
public class LaporanLogBarangController {
    ...
    private void getLogBarang(){
        if(barangTable.getSelectionModel().getSelectedItem()!=null){
            try(Connection con = Koneksi.getConnection()){
                List<LogBarang> listLogBarang = LogBarangDAO.getAllByDateAndBarang(con, 
                ...
                Collections.sort(listLogBarang, (o1, o2) -> {
                ...
                JRBeanCollectionDataSource beanColDataSource = new JRBeanCollectionDataSource(listLogBarang);
                ...
                zoomLevel.valueProperty().addListener((ObservableValue<? extends Number> observable, Number oldValue, Number newValue) -> {
                    zoomFactor = newValue.doubleValue() * 1.2 / 100;
